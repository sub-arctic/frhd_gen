...
{
    cv::Mat image = cv::imread(imagePath, grayscale ? cv::IMREAD_GRAYSCALE
                                                    : cv::IMREAD_UNCHANGED);
    if (image.empty())
    {
...
// Performance is calculated as O(m * n), where m is the number of rows in the
// image, and n is the number of columns
                                 ...
                                 const int &grayThreshold)
{
    ...
    for (int row = 0; row < grayImage.rows; ++row)
    {
        ...
        for (int col = 0; col < grayImage.cols; ++col)
        {
            ...
            else
            {
                if (count > 0)
                {
                    ...
                    if (col < grayImage.cols - 1)
                        neighbors++; // right neighbor

                    // adjust the run coordinates if there are fewer than 2
                    // neighbors
        ...
        if (count > 0)
        {
            ...
            int neighbors = 0;
            if (grayImage.cols - count - 1 >= 0)
                neighbors++; // Left neighbor
            if (grayImage.cols - 1 < grayImage.cols)
                neighbors++; // Right neighbor
